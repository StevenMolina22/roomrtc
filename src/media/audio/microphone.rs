...
impl Microphone {
    ...
    pub fn start(&mut self) -> Result<Receiver<AudioFrame>, Error> {
        ...
        self.logger.info(&format!(
            "Native microphone: {}Hz, {} channels. Resampling to {}Hz.",
            input_rate, input_channels, self.config.media.audio_sample_rate
        ));

        let stream = self.generate_input_stream(device, config, input_channels, input_rate, tx)?;

        stream.play().map_err(|e| Error::MapError(e.to_string()))?;
        *self
            .stream
            .lock()
            .map_err(|e| Error::MapError(e.to_string()))? = Some(stream);
    ...
    fn convert_to_mono(data: &[f32], channels: usize) -> Vec<f32> {
        match channels {
            ...
            2 => data
                ...
                .collect(),
            _ => data.chunks(channels).map(|c| c[0]).collect(),
        }
    }
