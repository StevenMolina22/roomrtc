...
impl Socket for MockSocket {
    fn send(&self, buf: &[u8]) -> Result<usize, std::io::Error> {
        self.sent_data.lock().map_err(|_| std::io::Error::new(std::io::ErrorKind::Other, "Failed to acquire lock due to poisoning"))?.push(buf.to_vec());
        Ok(buf.len())
    }
    ...
    fn recv_from(&self, buf: &mut [u8]) -> Result<(usize, std::net::SocketAddr), std::io::Error> {
        if let Some(data) = self.data_to_receive.first() {
            ...
            Ok((len, "127.0.0.1:1234".parse().map_err(|_| std::io::Error::new(std::io::ErrorKind::InvalidInput, "Failed to parse socket address"))?))
        } else {
