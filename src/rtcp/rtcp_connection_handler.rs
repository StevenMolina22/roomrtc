...
impl RTCPReportHandler {
    pub fn new(socket: UdpSocket, connection_status: Arc<RwLock<ConnectionStatus>>) -> Self {
        Self { socket, connection_status }
    }

    pub fn start(&self) -> Result<(), RTCPError> {
        let sender_socket = self.socket.try_clone().map_err(|_| RTCPError::SocketCloneFailed)?;
        let receiver_socket = self.socket.try_clone().map_err(|_| RTCPError::SocketCloneFailed)?;
    ...
    fn start_report_sender(&self, report_socket: UdpSocket) {
        ...
        thread::spawn(move || {
            loop {
                ...
                if report_socket.send(RTCPPackage::ConnectivityReport.as_bytes()).is_err() {
    ...
    fn start_report_receiver(&self, report_socket: UdpSocket) -> Result<(), RTCPError> {
        report_socket.set_read_timeout(Some(REPORT_RECEIVE_LIMIT)).map_err(|_| RTCPError::SocketConfigFailed)?;
        let shared_connection_status = Arc::clone(&self.connection_status);
...
fn try_receive_report(
    ...
    match report_socket.recv_from(&mut buf) {
        Ok((size, _src_addr)) => {
            ...
            match RTCPPackage::from_bytes(&buf[..size]) {
        ...
        Err(_) => {
            if Local::now() - *last_report_time > chrono::Duration::from_std(REPORT_RECEIVE_LIMIT).unwrap() {
