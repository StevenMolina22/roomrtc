...
pub fn setup_peer() -> TestPeer {
    ...
    let config = Arc::new(Config::load(Path::new("roomrtc.conf")).unwrap());

    let mut controller = Controller::new(tx_local, tx_remote, tx_event, config.clone()).unwrap();

    // Make a mock camera and swap it with the real one
    let mock_cam = MockCamera::new(rx_for_camera);
    controller.camera = Arc::new(Mutex::new(mock_cam));
