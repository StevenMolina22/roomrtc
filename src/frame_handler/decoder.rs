...
mod tests {
    ...
    #[test]
    fn test_decoded_frame_has_the_expected_size() {
        let mut cam = videoio::VideoCapture::new(0, videoio::CAP_ANY).unwrap();
        assert!(videoio::VideoCapture::is_opened(&cam).unwrap());

        let mut mat = Mat::default();
        cam.read(&mut mat).unwrap();
        assert!(!mat.empty());
        ...
        let yuv_bytes = yuv.data_bytes().unwrap().to_vec();
        let yuv_buff = YUVBuffer::from_vec(yuv_bytes, width as usize, height as usize);

        let mut encoder = H264Encoder::new().unwrap();
        let encoded = encoder.encode(&yuv_buff).unwrap().to_vec();

        let mut decoder = Decoder::new().unwrap();
        let decoded_rgb = decoder.decode_frame(&encoded).unwrap();
